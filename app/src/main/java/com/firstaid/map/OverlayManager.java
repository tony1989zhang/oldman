...
public abstract class OverlayManager implements OnMarkerClickListener, OnPolylineClickListener {
     ...
     * @return 管理的Overlay列表
     */
    public abstract List<OverlayOptions> getOverlayOptions(int iconId);
     ...
     * 将所有Overlay 添加到地图上
     */
    public final void addToMap(int iconId) {
        if (mBaiduMap == null) {
            return;
        }

        removeFromMap();
        List<OverlayOptions> overlayOptions = getOverlayOptions(iconId);
        if (overlayOptions != null) {
            mOverlayOptionList.addAll(getOverlayOptions(iconId));
        }
